...
acceptable during `Phase I`, and only due to securtiy issues.
...
It wasn't listed in global-requirements but it is required.  In order to block
the affected releases and still be able to keep requirements in sync, we list
the library in global-requirements.txt and arequired projects.
