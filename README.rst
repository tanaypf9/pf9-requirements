...
- Commit message should refer to consuming projects(s)
  ...
  All OpenStack projects, regardless of status, may add entries to
  ``global-requirements.txt`` for dependencies if the project is going
  to run integration tests under a devstack-configured environment. We
  want everyone testing with the same requirements, and any project
  that wants to test in a fully configured environment needs to have
  their dependencies in the global list.
