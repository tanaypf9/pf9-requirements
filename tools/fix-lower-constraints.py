...
Instructions:
 ...
 5. /path/to/local/copy/of/requirements/repository/tools/fix-lower-constraints.py > new-lc.txt
...
for const in constraints.values():
    ...
    if const.package not in requirements:
        # Ignore secondary dependencies
        output.append(const.to_line())
        continue
    for req, _ in requirements[const.package]:
        min = [
            s
