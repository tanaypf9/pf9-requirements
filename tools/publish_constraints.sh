...
function get_from_git()
{
    ...
    git show origin/${ref}:upper-constraints.txt > ${path}/${series}.txt
}

# Make the directory tree, don't fail if it already exists
mkdir -p publish/constraints/upper || true
# Clear out the directory tree, don't fail if we just created it
rm publish/constraints/upper/* || true

case "$ZUUL_BRANCH" in
stable/*)
    series=$(basename "$ZUUL_BRANCH")
    get_from_git $ZUUL_BRANCH ${series}
;;
master)
    # NOTE(tonyb): Publish EOL'd constraints files.  We do this here as a quick
    # way to publish the data.  It can be removed anytime after the first
    # successful run
