...
def _make_sort_key(line):
    # Remove some of the more common parts of names to produce sort
    # values that are less likely to put all of the oslo libraries
    # together and all of the client libraries together.
    dep = line.partition('=')[0].lower()
    for prefix in ['xstatic-angular-', 'xstatic-', 'os-', 'oslo.', 'python-']:
        if dep.startswith(prefix):
            dep = dep[len(prefix):] + '-' + prefix[:-1]
            break
    return dep
...
def main(argv=None, stdout=None):
    ...
    frozen = sorted(
        _combine_freezes(freezes, blacklist),
        key=_make_sort_key
    )
    ...
    #
    # When adding a new entry by hand, sort as though the name
    # did not start with xstatic-angular-, xstatic-, os-, oslo.,
    # or python- and ignore case.
    #
    '''))
